...
import { Smile, MessageCircle, CheckCircle, TrendingUp, Play, Users, Target, Sparkles, DollarSign, Filter, X, ChevronDown, Calendar, Radio, MapPin, Instagram, Facebook, Globe, Leaf, Gift, BarChart3, LucideIcon } from 'lucide-react';
...
// Channel options for the filter with real icons
const CHANNEL_OPTIONS: { value: FunnelChannel; label: string; Icon: LucideIcon; color: string }[] = [
  { value: 'all', label: 'Все', Icon: BarChart3, color: '#6366F1' },
  { value: 'instagram', label: 'Instagram', Icon: Instagram, color: '#E1306C' },
  { value: 'facebook', label: 'Facebook', Icon: Facebook, color: '#1877F2' },
  { value: 'google', label: 'Google', Icon: Globe, color: '#4285F4' },
  { value: 'organic', label: 'Органика', Icon: Leaf, color: '#22C55E' },
  { value: 'promo', label: 'Промо', Icon: Gift, color: '#F59E0B' },
];
          ...
          <div className="flex items-center gap-1.5">
              ...
              <div className="relative">
                <button
                  ...
                  `}
                >
                  {(() => { const ch = CHANNEL_OPTIONS.find(c => c.value === selectedChannel); const ChIcon = ch?.Icon || Radio; return <ChIcon className="w-3.5 h-3.5" style={{ color: ch?.color }} />; })()}
                  <span className="hidden sm:inline">{selectedChannel !== 'all' ? CHANNEL_OPTIONS.find(c => c.value === selectedChannel)?.label : 'Канал'}</span>
                      ...
                      {CHANNEL_OPTIONS.map(channel => {
                        ...
                        return (
                          <button
                            ...
                            `}
                          >
                            <ChannelIcon className="w-4 h-4" style={{ color: channel.color }} />
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{funnelFilter.managerId ? MOCK_MANAGERS.find(m => m.id === funnelFilter.managerId)?.name.split(' ')[0] : 'Менеджер'}</span>
                  <ChevronDown className="w-3 h-3" />
                </button>
                <AnimatePresence>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="hidden sm:inline">{funnelFilter.trafficTeamId ? MOCK_TRAFFIC_TEAMS.find(t => t.id === funnelFilter.trafficTeamId)?.name.split(' ')[0] : 'Команда'}</span>
                  <ChevronDown className="w-3 h-3" />
                </button>
                <AnimatePresence>
